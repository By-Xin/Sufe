...
- ADF的基本模型
  $$y_t = \beta_0 + \beta_1 y_{t-1} + \cdots + \beta_py_{t-p}+\gamma t+\epsilon_t ~...(\ast)$$
  - 其中$Y_t$即为这里想要分析的时间序列，ADF检验因此相当于构建了一个AR(p)模型

- 上述模型的恒等变形
  $$y_t = \beta_0 + \rho y_{t-1}+\gamma_1\Delta y_{t-1}+\cdots+\gamma_{p-1}\Delta y_{t-p-1}+\gamma t+\epsilon_t~...(\ast\ast)$$
    - 其中$\rho, \gamma_1,\cdots,\gamma_{p-1}$ 为一系列待定系数
    - 这里相当于对于$(\ast)$，一定可以将其写作上式，其具体取值也可以直接通过代数并对应项相等的方式完成，具体取值这里忽略

- 关于单位根的讨论
  - 若$\rho =1$，则AR(p)有一个单位根
  - 若$\rho>1$，则AR(p)至少有一个根在单位圆内，说明AR(p)非平稳

- ADF的假设检验
  - $H_0:\rho=1$，即AR(p)有一个单位根
  - $H_1:\rho<1$，即AR(p)既没有单位根，又不是非平稳的，即AR(p)是平稳的

- ADF的最终计算模型
  - 对$(\ast\ast)$左右同减$y_{t-1}$，有
  $$\Delta y_t = \beta_0 + \delta y_{t-1}+\gamma_1\Delta y_{t-1}+\cdots+\gamma_{p-1}\Delta y_{t-p-1}+\gamma t+\epsilon_t~...(\star)$$
  - 其中$\delta \equiv \rho -1$
  - 假设检验变为
    - $H_0:\delta =0$ 模型存在单位根（一阶差分后平稳）
    - $H_0:\delta < 0$ 模型是平稳的
  - ADF统计量
    - 对$(\star)$进行OLS回归，即可以得到$\hat\delta$和OLS参数检验的t统计量，该统计量即为ADF统计量
    - 该统计量的分布p值无法求得解析解，需要通过蒙特卡洛模拟的方法得到其数值解

- 模型的参数选择
  - 常数项与时间趋势项
    - 主要从理论上进行考虑
    - 可以通过时间序列图大致判断是否有长期增长趋势
    - 也可以通过判断常数项和时间项是否显著
    - 如果无从判断，为了稳健起见，也可以都进行检验，将结果列于表中
  - 滞后阶数的选择
    - ADF中滞后阶数$p$的选择是一个empirical的问题
    - 如果$p$太小，则会存在检验偏差；反之若过大则会导致检验的势（power）降低
    - Schwert(1989)建议选取最大滞后阶数为
      $$p_{\max}=[12(T/100)^{1/4}]$$
        - 其中$[\cdot]$表示取整，T为样本容量
