...
# slice the data
dat4.gp1 <- as.matrix(dat4.train %>% filter(g==1) %>% select(-g))
dat4.gp2 <- as.matrix(dat4.train %>% filter(g==2) %>% select(-g))
dat4 <- as.matrix(dat4.train %>% select(-g))

# calculate mean & var 
mu1 <- apply(dat4.gp1, 2, mean)
mu2 <- apply(dat4.gp2, 2, mean)
var1 <- cov(dat4.gp1)
var2 <- cov(dat4.gp2)

# calculate the distance
Pr1 <- 0; Pr2 <- 0; PlayerGrp <- 0; x <- 0
for (player in 1:14){
  x <- as.matrix(dat4.test[player,])
  dis1 <- (x- mu1) %*% solve(var1,t(x-mu1))
  dis2 <- (x- mu2) %*% solve(var2,t(x-mu2))
  Pr1 <- exp(-0.5*dis1)
  Pr2 <- exp(-0.5*dis2)
  if ( Pr1/(Pr1+Pr2) < Pr2/(Pr1+Pr2) ){
    PlayerGrp[player] = 2
  }
  if ( Pr1/(Pr1+Pr2) > Pr2/(Pr1+Pr2) ){
    PlayerGrp[player] = 1
